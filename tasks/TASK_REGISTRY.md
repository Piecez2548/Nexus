...
>
> `tsc -b`, `oxlint`, and the full test suite (2153 tests, up from 2147) are clean; `npm run build` succeeds. **Device-only verification — NOT performed, explicitly not claimed**: whether `onNotificationPosted` actually fires for a real payment on a real device; whether the three package-name guesses (`com.scb.phone`, `com.kasikorn.retail.mbanking.wap`, `com.ktb.next`) are correct; what the real notification title/text/bigText actually contain per bank app and transaction type; whether the tap-to-resume flow behaves correctly across cold/warm/hot app states. None of this can be simulated in Vitest. **Concrete next step, before trusting any bank's parser rules**: install the target bank app, trigger a real transaction, and run `adb shell dumpsys notification --noredact` to read the exact real notification content the listener would see — repeat per bank, in Phase-1 order, before Phase 2 adds the remaining banks.
