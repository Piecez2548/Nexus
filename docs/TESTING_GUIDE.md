...
- `npx vitest run src/features/sync/syncEngine.test.ts src/features/sync/components/SyncProvider.test.tsx src/features/sync/tombstones.test.ts src/features/sync/store/authStore.test.ts`: sync propagation, deletion safety, signed-in orchestration, user-filtered Realtime wake-up, busy-pass event coalescing, bounded four-request pull batches, and the five-second/background-online recovery triggers. The two-device regression alternates isolated desktop/mobile local states against one stateful cloud relay in both directions.
