...
- `npx vitest run src/features/sync/syncEngine.test.ts src/features/sync/components/SyncProvider.test.tsx src/features/sync/tombstones.test.ts src/features/sync/store/authStore.test.ts`: sync propagation, deletion safety, signed-in orchestration, user-filtered Realtime wake-up with a whitelisted changed-table hint (including an UPDATE hint carried on the old row), targeted single-table pulls, busy-pass event coalescing, bounded four-request full-pass batches with preferred-table ordering, and the five-second/background-online recovery triggers. The two-device regression alternates isolated desktop/mobile local states against one stateful cloud relay in both directions.
- The auth/provider tests also verify privacy-safe monitoring context for sync failures and Realtime channel errors; no user ID or encrypted payload is included in the extra fields.
