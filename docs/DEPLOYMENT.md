...
**Server-side environment (Supabase project, not this repo):** `supabase/schema.sql` must be run once in the target Supabase project's SQL Editor to create the `synced_records`, `user_encryption_keys`, `mfa_backup_codes`, `ai_coach_daily_usage`, and `automation_weekly_digests` tables + RLS policies before sync/encryption/2FA/the AI Coach fallback/the weekly digest will function against that project — see [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md).

**Dashboard-only, one toggle:** the weekly financial digest requires the **`pg_cron`** extension (Supabase Dashboard → Database → Extensions → enable `pg_cron`) so `generate_weekly_digests()` actually runs on its schedule. No external account, no credentials — just a checkbox. `schema.sql` is safe to run before or after enabling it (the `cron.schedule(...)` call it ends with is wrapped to skip gracefully and print a reminder if `pg_cron` isn't enabled yet — re-run `schema.sql` once after flipping the toggle to actually schedule the job).
...
**Mostly not applicable — no persistent application server exists or is deployed from this repository.** Supabase's managed Auth + Postgres remain configured via the environment variables above, not deployed here. Two exceptions exist, neither a persistent server: `supabase/functions/ai-coach` (see above), one small stateless Edge Function deployed via the Supabase CLI; and `generate_weekly_digests()`, a Postgres function invoked on a schedule by the `pg_cron` extension (a Dashboard toggle, not a deploy step at all). Neither changes this document's overall story — there is still no persistent application server, no API gateway, and no deployment pipeline in this repository for anything beyond the three client targets documented above. See [PROJECT_ARCHITECTURE.md](PROJECT_ARCHITECTURE.md)'s "Future Backend Architecture" and [DECISIONS.md](DECISIONS.md) for the fuller picture.
