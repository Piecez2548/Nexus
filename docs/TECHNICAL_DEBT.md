...
- **The Gallery Slip Scanner (GS epic) is built and tested but not yet wired into the running app — deliberately incremental.** The full headless pipeline now exists and is tested (159 slipScanner tests as of GS-021): scan orchestration + queue + cache (GS-005–GS-008), the extraction engine QR → EMVCo → bank → OCR-fallback → dedup (GS-009–GS-013), the `SlipCandidate` model + Import Preview + Smart Import (GS-014–GS-016), and the security/perf/validation/analytics layers (GS-017–GS-020). Two concrete gaps remain, both tracked, not shortcuts: (1) the `NativeMediaProvider` is still a stub, so there is no on-device gallery enumeration until a Capacitor media plugin is wired — the scanner is web-file-picker-only; and (2) none of the scanner UI (`BankSelectionPopup`, `ImportPreview`) is mounted on a route or reachable from the app yet, so the feature is not user-facing. Smaller placeholders: the scan-cache engine versions are `"0"` and `SlipCandidate.confidence` is a transparent completeness heuristic (`basicConfidence`) pending the dedicated Confidence Engine (GS-046). Flagged here so the module isn't mistaken for a shipping feature. See [../tasks/TASK_REGISTRY.md](../tasks/TASK_REGISTRY.md)'s GS epic.
