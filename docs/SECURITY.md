...
## AI Coach — Claude Fallback (`src/ai/providers/ClaudeProvider.ts`, `supabase/functions/ai-coach`)

An opt-in, real-LLM fallback for the AI Coach (Settings > Security & Sync > AI Coach), used only when the rule-based coach's `classifyIntent()` returns `"unknown"` for a question — every rule-answerable question keeps its existing deterministic, offline, zero-cost answer path untouched (see "Why is Rule-based, not LLM-based, AI?" in [DECISIONS.md](DECISIONS.md)).

- **Off by default, three independent gates.** The fallback only ever fires when all three are true: the user explicitly enabled it in Settings (`aiCoachSettingsStore`, persisted locally, defaults to `false`), cloud sync is configured for the build (`isSyncConfigured`), and the user is actually signed in. Any one being false silently keeps the existing static "I don't understand" response — never a crash, never a hang.
- **The API key never reaches the client.** `ANTHROPIC_API_KEY` exists solely as a secret on the `ai-coach` Supabase Edge Function (`supabase secrets set ANTHROPIC_API_KEY=...`), never as a `VITE_` build variable — `VITE_AI_PROVIDER` only selects *which* provider the Gateway uses, and holds no credential of its own.
- **JWT-verified, no service-role key.** The Edge Function is this project's first server-side code, and it deliberately follows the same access-control model as every other Supabase table in this app: it forwards the caller's own session JWT into a Supabase client scoped to that user, calls `auth.getUser()` to confirm it, and every subsequent database operation runs as that user under Row-Level Security — there is no service-role key anywhere in this feature, and a request with a missing/invalid JWT is rejected with `401` before anything else happens.
- **Rate-limited server-side, not just client-side.** A per-user, per-UTC-day counter (`public.ai_coach_daily_usage`, RLS-scoped `auth.uid() = user_id`) is atomically incremented via a `SECURITY INVOKER` Postgres function before the Edge Function ever calls Anthropic, so a capped user's request never reaches (or gets billed by) the upstream API, and a user can only ever see or increment their own counter — checked and reasoned about explicitly, since this is the one place in the app where an uncapped bug could turn into real, uncontrolled third-party spend.
- **Only a redacted summary ever leaves the device.** `buildCoachLlmContext()` builds a strict allow-list of round numbers and category-/behavior-style-level aggregates (income/expense/savings totals, top categories, budget status, health score, spending-style label, generic habit flags) — it never includes a named merchant, an individual transaction's title/date, or anything else identity-bearing from the user's real financial history. This is verified by a dedicated test that asserts a distinctive merchant name and transaction title are absent from the built payload, not just that the expected fields are present.
- **The model is told not to fabricate.** The Edge Function's system prompt instructs Claude to answer only from the provided JSON summary, say so plainly if the summary is insufficient rather than guess, and decline anything outside the user's own personal finances — the same "never fabricates" discipline the rule-based Coach already documents for itself, carried into the LLM path as an explicit instruction rather than assumed.
- **Every LLM-sourced answer is visually distinguished**, not presented as equally certain as a deterministic one — `AiCoachAnswerCard.tsx` shows an "AI-generated" badge instead of a (meaningless, for free-form prose) confidence percentage whenever an answer came from this fallback.

## Automation — Weekly Digest (`public.generate_weekly_digests()`, `src/features/automation/`)

A server-computed weekly financial summary, generated on a schedule (`pg_cron`) so it's ready the next time the app opens, even if it was closed all week — the first place in this app where a Postgres function reads across user boundaries rather than every access being scoped to the calling user's own row.

- **`SECURITY DEFINER`, deliberately, and only here.** Every other custom Postgres function in this app (`increment_ai_coach_usage()`) is `SECURITY INVOKER` — no more power than the calling user's own RLS grants. `generate_weekly_digests()` is the one exception, because its entire job — computing a digest for every eligible user in one scheduled pass — is impossible for an RLS-respecting invoker role to do at all. It is never callable by a client (no `execute` grant to `authenticated`/`anon`; only `pg_cron`'s own scheduling role invokes it), and its `search_path` is pinned per Postgres's own `SECURITY DEFINER` hardening guidance.
- **Read+update only from the client, never insert or delete.** `automation_weekly_digests`'s RLS policies cover `select` (a user's own rows) and `update` (marking `seen_at`, the only field a client ever writes) — there is no insert or delete policy for the `authenticated` role at all, so a user can never fabricate or erase a digest; only the scheduled function creates rows.
- **Never computed for an encrypted account — not filtered as an afterthought, structurally impossible.** The function excludes any `user_id` present in `public.user_encryption_keys` (row presence there reliably means "this account has encryption enabled," since `disableEncryption.ts` deletes that row on disable) before it ever aggregates a single `synced_records` row. For an encrypted account, `synced_records.data` is ciphertext — even without the exclusion, the server has no way to read it. This was a deliberate, surfaced trade-off during scoping, not a bug: no report, no partial report, no error — the card simply never appears for that account. See [DECISIONS.md](DECISIONS.md).
- **Malformed data can't take down another user's digest.** The generator query only ever aggregates rows whose `type`/`amount`/`date` fields already match a strict regex/enum shape before casting — one unexpected row (from a bug, or a future schema change this function hasn't caught up with) is silently excluded from that user's own sum rather than throwing and aborting the whole scheduled batch.
- **No new external dependency.** Unlike the AI Coach (Anthropic account, API key, Edge Function deploy), this only needs the `pg_cron` extension — a single Supabase Dashboard toggle, no credentials, no third party.
