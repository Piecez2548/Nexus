...
- **Gallery Scanner: a way to correct a slip's guessed category** — `categoryLearningStore` (GS-045) and its read-side wiring into import both exist and work, but no screen exposes a category field/picker for a scanned slip, so nothing ever writes a correction.
