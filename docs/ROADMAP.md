...
- **Gallery Slip Scanner** (`GS` epic — see [../tasks/TASK_REGISTRY.md](../tasks/TASK_REGISTRY.md) and `MASTER_TASK.md`) — a production, plugin-agnostic gallery scanner is being built out incrementally. Landed so far (local, not yet released): the scan foundation — permission manager (GS-005), scan orchestration with a `MediaProvider` adapter (GS-006), concurrent queue with retry + memory budget (GS-007), versioned scan cache (GS-008); the full extraction pipeline — QR detection (GS-009), EMVCo/PromptPay payload parser (GS-010), plugin-based bank identification (GS-011), OCR fallback reusing the existing Tesseract engine (GS-012), slip-level duplicate detection (GS-013), the unified `SlipCandidate` model + Import Preview (GS-015), and Smart Import with batch/progress/resume/rollback (GS-016); plus the supporting layers — bank-selection popup (GS-014), security utilities (audit / secure deletion / tamper detection, GS-017), performance metrics (GS-018), deterministic AI validation that never mutates data (GS-019), cross-run analytics (GS-020), and critical-path integration + stress tests (GS-021), reviewed at the GS-022 checkpoint. It is now **wired into the app on web** — a "Scan Gallery" button on the Transactions page runs the real flow (bank select → pick images → jsQR + Tesseract extraction → Import Preview → Smart Import); native gallery picking is wired via `@capacitor/camera` and a debug APK is installed on-device (pending on-device verification). A second wave of refinement engines then landed (GS-023–GS-039): a battery/charging-aware scan scheduler, an image hash engine (SHA-256 + perceptual hash), a slip validation engine, QR recovery (rotate/enhance retries), image enhancement, a richer per-field OCR engine, a slip classifier, a plugin-based bank template engine, a smart (graded-probability) duplicate engine, an import conflict resolver, a general background worker, a scan-progress dashboard, import history (Dexie v17), a performance monitor, a crash/interruption recovery system, a security audit layer, and dev tools — reviewed at the GS-040 production-readiness checkpoint. The AI/intelligence layers then completed the epic (GS-041–GS-050): AI slip verification, fraud detection, transaction categorization (with learning), merchant intelligence, a smart learning engine, a confidence engine, transaction linking, spending intelligence, AI quality review, and a financial intelligence report — **all deterministic, local, and advisory (never mutating imported data)**. **The GS epic is complete (50/50, ~297 slipScanner tests).** The `PLT` platform epic then landed: the genuinely-new frameworks were implemented — Event Bus, Feature Flags, Command Palette (Ctrl+K), Local Telemetry (`src/platform/`) — and the rest mapped to existing app systems or design (see [../tasks/Platform/PLATFORM_DESIGN.md](../tasks/Platform/PLATFORM_DESIGN.md)), certified at PLT-020. The one remaining functional gap is **full-gallery *auto*-enumeration** (the `NativeMediaProvider` MediaStore path is still a stub — today's shipping path is the file/photo picker) and **on-device verification** of the native picker.
