...
**Two scan paths coexist by design:** the shipping picker flow (`GalleryScanFlow`/`useSlipScan`, sequential, same-batch near-duplicate detection computed inline as each candidate arrives) and the concurrent orchestrator path (`FullGalleryScanPanel`/`useFullGalleryScan`, built for a full native-gallery scan) — the latter can't compute the badge inline the same way, since that depends on a strict processing order the concurrent queue doesn't guarantee. It instead runs `flagBatchDuplicates()` once, after the scan settles, over the complete accumulated batch sorted by the one stable key every candidate carries (`assetId`) — exact-match only (payload/reference/amount/timestamp/merchant/bank), not the picker flow's additional perceptual-hash check, since that needs the original image bytes, never kept on a `SlipCandidate` at full-gallery scale.
