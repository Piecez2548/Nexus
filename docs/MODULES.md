...
**Future Plans:** None currently scoped for this module — see [ROADMAP.md](ROADMAP.md)'s Planned section for what remains app-wide (AI Gateway integration).
...
**Also includes:** a versioned scan cache and concurrent, byte-budgeted queue (`ScanCache`/`MediaProvider`, `scanSessionService`) built for a 50k-image library, now driving real extraction (`slipExtractionProcessor`) through the live `GalleryScanFlow` and `useFullGalleryScan` path — including progress, pause/resume/cancel, native gallery enumeration, and the web file-picker fallback; Smart Import with progress/cancel/resume/rollback into the existing `transactionService`, plus conflict resolution against existing transactions; security (audit log, secure deletion, CRC tamper detection); a deterministic, advisory (never-mutating) AI layer — slip verification, fraud detection, transaction categorization with local learning (now with a Review Queue UI to correct a guess and have it stick), merchant intelligence, transaction linking, spending intelligence, a financial intelligence report.

**The shipping scan path is unified:** `GalleryScanFlow` uses `useFullGalleryScan` for both native full-gallery enumeration and the web file-picker fallback. Because the concurrent queue cannot depend on arrival order, it runs `flagBatchDuplicates()` after the scan settles over the complete batch sorted by stable `assetId`, then automatically runs Smart Import on the bank-filtered candidates. Cross-batch duplicate protection still runs during Smart Import.

**Current Status:** Complete (GS epic 50/50) and verified on-device (Android APK installed and tested against real slips, with two follow-up bug-fix/code-review rounds — see [../tasks/TASK_REGISTRY.md](../tasks/TASK_REGISTRY.md)'s "Post-Launch Stabilization"). The 2026-08-15 Slip Intelligence Phases 1-9 closed the confidence-engine, smart-duplicate, conflict-resolver, recovery-system, and category-learning-UI gaps, added a confidence-tier import policy and an Import History screen, and implemented the native `GalleryMediaPlugin.java` MediaStore plugin — since device-validated, with the "Scan Gallery" button on the Transactions page now driving the orchestrator path directly (`GalleryScanFlow`) — see [TASK_REGISTRY.md](../tasks/TASK_REGISTRY.md)'s "Live wiring + real-device tap-through" entry.

**Future Plans:** See [ROADMAP.md](ROADMAP.md) and [../tasks/TASK_REGISTRY.md](../tasks/TASK_REGISTRY.md) for full detail (this module has no dedicated architecture doc the way AI Analytics does — the task registry's per-task notes serve that role).
...
**How it works:** one `Subscription` model — `{name, amount, billingFrequency, nextBillingDate, status, category?, account?, note?, icon, color, reminderEnabled?, billingAnchorDay?}` — where `billingFrequency` reuses the existing `RecurringFrequency` type (no new frequency enum) and `status` (`"active" | "paused" | "cancelled"`) is new, with no direct precedent elsewhere in the app. `nextBillingDate` is stored exactly as entered/edited and is **never silently rewritten by display logic**; `subscriptionMath.ts`'s `resolveNextBillingDate()` is a pure function that rolls it forward for *display only*, using `date-fns`'s calendar-aware `addDays/addWeeks/addMonths/addYears` (correctly handling edge cases like Jan 31 + 1 month → Feb 28). `calculateSubscriptionStats()` (also in `subscriptionMath.ts`) sums active-only monthly/yearly totals and per-status counts, feeding both `useSubscriptionStats()` (the page) and `SubscriptionSummaryGrid`.

**Billing anchor day (BUG-12).** A month-length clamp (Jan 31 + 1 month → Feb 28) is correct in isolation, but chaining the *next* advance from that already-clamped date used to lose the original day forever — every later cycle permanently rebased to the 28th instead of returning to the 31st once the calendar allowed it (Mar 31, Apr 30, May 31, ...). `billingAnchorDay` (optional) persists the true intended billing day separately from `nextBillingDate`, resolved once per subscription (`resolveBillingAnchorDay()`, falling back to `nextBillingDate`'s own day when absent) and threaded unchanged through every cycle in both `advanceOneBillingCycle()`'s and `resolveNextBillingDate()`'s loops — so the anchor is never re-derived from an already-clamped intermediate value. `SubscriptionForm.tsx` (re)computes it only when the user actually changes `nextBillingDate`, preserving a correctly-tracked anchor across an edit to any other field; `subscriptionTransactionService.ts`'s catch-up loop backfills it onto the persisted row the first time a legacy subscription (one created before this field existed) passes through it. No Dexie schema version was needed — `billingAnchorDay` is a plain, unindexed field on the already-synced-and-encrypted `subscriptions` table, so it rides along through sync/encryption/backup automatically. A subscription whose day had *already* drifted before this field existed cannot have its true original anchor recovered (that information no longer exists anywhere) — the fallback is honestly "whatever day it shows today," not a recovered original.
...
**Route:** none (no dedicated page — surfaced via `SyncSettings.tsx`/`MfaSettings.tsx`/`LoginHistorySettings.tsx` and the sign-in gate wrapping the whole app).

**Features:** email/password sign-up/sign-in (no OAuth, no magic link); optional TOTP two-factor authentication and one-time backup codes for the account, using Supabase Auth's own native MFA API directly (no new backend); a generic push/pull sync engine against one Supabase table (`synced_records`) covering 17 of the app's tables (`merchants` excluded — seed data, not personal); tombstone-based deletion propagation; last-write-wins conflict resolution on a client-embedded timestamp, enforced atomically on live cloud updates; a malformed-row structural guard on pull; periodic (5s) + `online`-event-triggered sync; graceful no-auth-screen fallback when Supabase env vars are absent, so the app never locks a user out with no configured way in; a one-time, flag-gated self-healing migration (`repairStuckPushCursorsOnce()`) that clears every table's local push cursor to repair a since-fixed bug where an older build's pull-side cursor nudge could silently exclude an unpushed local row from every future sync pass — repeated older writes are rejected by the database live-version guard and terminal tombstones remain authoritative.

**Two-factor authentication (EXEC-adjacent Tier-4 item, `mfa.ts`/`backupCodes.ts`/`mfaSession.ts`):** enrollment shows a QR code + setup secret from `auth.mfa.enroll()`, confirmed via `auth.mfa.challengeAndVerify()`; the same primitive verifies a code at the sign-in challenge screen (`MfaChallengeScreen.tsx`, shown by `AuthGate` in place of `LoginScreen`). Session gating is tracked by the app itself (`sessionStorage`-scoped "verified" flag), not solely by Supabase's own AAL — necessary because backup-code redemption is a custom mechanism Supabase's own assurance-level check can't see, so relying on that alone would re-prompt in a loop after a valid backup-code sign-in. `resolveMfaAccess()` is the single gate, called from every place `authStore.ts` can set the signed-in user (session-restore, the `onAuthStateChange` listener, and `signIn()`), so a password-only session can never race past the challenge regardless of path. Backup codes live in a new Postgres table (`public.mfa_backup_codes`, see [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md)), hashed with the same salted-SHA-256 helper the App Lock PIN already uses, verified entirely client-side.

**Components:** `AuthGate` (top-level gate), `LoginScreen`, `MfaChallengeScreen`, `EnrollMfaForm`, `DisableMfaForm`, `RegenerateBackupCodesForm`, `BackupCodesDisplay`, `SyncProvider` (side-effect-only mount).

**Services:** none — `syncEngine.ts`/`tombstones.ts`/`mfa.ts`/`backupCodes.ts` are free functions, not a service-layer wrapper.

**Stores:** `authStore` (Supabase session + sync status + MFA challenge state — not an entity cache, wraps the Supabase Auth SDK directly).

**Database usage:** reads/writes all 17 synced tables generically, plus `syncTombstones` and `syncState` (sync cursor bookkeeping). Backup codes are the one exception to "everything lives in local Dexie first" — `mfa_backup_codes` exists only server-side in Supabase Postgres (see [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md)), fetched live, never mirrored to a local table or included in `SYNCED_TABLES`/backup export.
