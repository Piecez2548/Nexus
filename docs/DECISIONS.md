...
**Follow-up:** that backend proxy now exists for one provider — `supabase/functions/ai-coach`, a single stateless Supabase Edge Function, backs a real `ClaudeProvider` used as an opt-in fallback for the AI Coach's `"unknown"`-intent questions only. This doesn't reverse the reasoning above: every other AI surface (scoring, analysis, recommendations, forecasting) stays rule-based, for the exact offline/zero-cost/zero-hallucination-risk reasons already argued. The Gateway's original design goal — a real remote provider pluggable through configuration alone — is what made this addition a config/wiring change (`ProviderFactory.ts`'s already-reserved `"claude"` slot) rather than a redesign.

## Why does the Weekly Digest only work for non-encrypted accounts?

Scoping this automation surfaced a real conflict between two of this app's own foundational decisions, not a limitation anyone chose lightly. "Automation... even while the app is closed" requires the computation to happen *server-side* (nothing runs on a closed client). But every financial-analysis engine Nexus has (`useFinancialAnalysis`, Reports, AI Analytics — see [AI_ANALYTICS.md](AI_ANALYTICS.md)) is 100% client-side, reading local Dexie directly; there is no server-side equivalent to call. Worse, for an account with encryption enabled, `synced_records.data` is AES-GCM ciphertext the server was specifically designed never to be able to read (see Layer 3 in [SECURITY.md](SECURITY.md)) — not a permissions gap to close, the entire point of that layer.

Resolved by scoping the automation itself, not by weakening encryption's guarantee: `public.generate_weekly_digests()` computes a *minimal* summary (income/expense/net/count only — nothing close to the real analysis engine's depth) directly from `synced_records`' plain JSON, and explicitly excludes any account present in `public.user_encryption_keys`. An encrypted account gets no digest, no error, no "unavailable" placeholder — the feature simply doesn't apply, the same way a feature gated behind a permission a user never granted just doesn't show up. This keeps the promise `docs/SECURITY.md`'s Layer 3 already makes (Supabase never has access to plaintext) intact without exception, at the cost of one automation not covering every account.
