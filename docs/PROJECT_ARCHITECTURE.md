...
**Sync path** (opt-in): a user-filtered Supabase Realtime change, the ~5s recovery timer, or the browser `online` event makes `SyncProvider` call `runFullSync(userId)`. Realtime includes a validated table hint; when present, that table is placed first in the initial deterministic four-table pull batch while the complete 24-table pass still runs. The client pushes locally-changed rows (`updatedAt` at/above the last push cursor) and tombstones to the generic Supabase table, then pulls remote changes with version and structural checks. Realtime events received during an active serialized pass coalesce into one follow-up pass. Independent pulls run in deterministic batches of four; each batch's results, errors and store refreshes are processed in table order. Pull tracks only successfully applied versions; it caps push-cursor advancement at any still-pending local version, querying and updating that cursor in one transaction with the entity table. The v2 one-time repair clears previously corrupted push cursors on existing installations. After a table applies remote changes, its Zustand store refreshes immediately instead of waiting for all later pulls. A final refresh still runs after account/category deduplication to reconcile dependent finance views. See [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md), [SC-003](SYNC_CONFLICT_FIX_003_2026-09-12.md), [the latency fix](SYNC_LATENCY_FIX_2026-09-13.md), [the Realtime trigger evidence](SYNC_TRIGGER_001_2026-09-13.md), [the pull-window evidence](SYNC_PASS_PERF_001_2026-09-13.md) and [the table-priority evidence](SYNC_PULL_PRIORITY_001_2026-09-13.md).
