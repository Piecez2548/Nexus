...
**Sync path** (opt-in): a user-filtered Supabase Realtime change, the ~5s recovery timer, or the browser `online` event makes `SyncProvider` start sync. Realtime includes a validated table hint; that path runs the same push, tombstone, cursor, conflict and dedupe guards but pulls only the hinted table. Timer, online and manual syncs retain the complete deterministic 24-table pass as the recovery boundary for missed, malformed or concurrent Realtime events. Realtime events received during an active serialized pass coalesce into one follow-up pass. Full-pass pulls run in deterministic batches of four; each batch's results, errors and store refreshes are processed in table order. Pull tracks only successfully applied versions; it caps push-cursor advancement at any still-pending local version, querying and updating that cursor in one transaction with the entity table. The v2 one-time repair clears previously corrupted push cursors on existing installations. After a table applies remote changes, its Zustand store refreshes immediately instead of waiting for all later pulls. A final refresh still runs after account/category deduplication to reconcile dependent finance views. `public.synced_records` uses `REPLICA IDENTITY FULL` so stable `table_name` survives UPDATE/DELETE Realtime payloads and valid foreground events stay on the targeted pull path. See [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md), [SC-003](SYNC_CONFLICT_FIX_003_2026-09-12.md), [the latency fix](SYNC_LATENCY_FIX_2026-09-13.md), [the Realtime trigger evidence](SYNC_TRIGGER_001_2026-09-13.md), [the pull-window evidence](SYNC_PASS_PERF_001_2026-09-13.md), [the table-priority evidence](SYNC_PULL_PRIORITY_001_2026-09-13.md), [the targeted-pull evidence](SYNC_TARGETED_PULL_001_2026-09-13.md) and [the Realtime diagnosis](SYNC_REALTIME_DIAG_001_2026-09-13.md).
...
- **Resilience:** every sync step is individually try/caught so one table's failure never blocks the others. A user-scoped Realtime subscription provides the normal foreground wake-up; the periodic timer and `online` listener recover if the socket is unavailable. There is no manual retry queue or offline write log beyond Dexie itself already being durable.
...
**Mostly not built.** There is still no application backend server or API gateway — Supabase is used for its hosted Postgres + Auth, accessed directly from the client via `@supabase/supabase-js`, with Postgres Row-Level Security as the only access-control layer (see [SECURITY.md](SECURITY.md)). The one exception is `supabase/functions/ai-coach`, a single stateless Supabase Edge Function that proxies the AI Coach's optional real-LLM fallback to Anthropic Claude, so the API key never lives in client code — it holds no business logic of its own (verifies the caller's JWT, atomically checks a per-user daily rate-limit counter via RLS, forwards the request to Anthropic, returns the text) and doesn't change the story above: there is still no server-side business logic for any of this app's actual domain data (transactions, budgets, trades, etc.), and RLS remains the only access-control layer everywhere, including inside that one function.
