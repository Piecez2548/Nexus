...
The complete 24-table pass remains the recovery boundary by design. Targeted mode is intentionally restricted to validated Realtime table names and is never used for timer, online or manual sync. The single-table pull and fallback routing are covered by the sync-engine, auth-store and provider regressions; mobile smoke verified the installed build and cleanup path without exposing encrypted payloads.
