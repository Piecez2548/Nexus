  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-4">
        <FormField label="สัญลักษณ์" htmlFor="trade-symbol" error={errors.symbol?.message}>
          <input
            id="trade-symbol"
            {...register("symbol")}
            placeholder="เช่น AAPL, XAUUSD, BTCUSD"
            className={inputClassName}
          />
        </FormField>

        <FormField label="ตลาด (AI ตรวจจับอัตโนมัติ)" htmlFor="trade-market">
      ...
      <FormField label="ทิศทาง" htmlFor="trade-direction">
      ...
      <div className="grid grid-cols-2 gap-4">
        <FormField label="ราคาเข้า" htmlFor="trade-entry-price" error={errors.entryPrice?.message}>
          <input
        ...
        <FormField label="Lot Size" htmlFor="trade-quantity" error={errors.quantity?.message}>
          <input
            id="trade-quantity"
            type="number"
            step="any"
      ...
      <div className="grid grid-cols-2 gap-4">
        <FormField label="วันที่เข้า" htmlFor="trade-entry-date" error={errors.entryDate?.message}>
          <input
            id="trade-entry-date"
            type="date"
            {...register("entryDate")}
        ...
        <FormField label="เวลาเข้า" htmlFor="trade-entry-time">
          <input
            id="trade-entry-time"
            type="time"
            {...register("entryTime")}
      ...
      <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
        ปิดรายการ (กรอกเมื่อออกจากตำแหน่งแล้ว)
      </p>

      <div className="grid grid-cols-2 gap-4">
        <FormField label="ราคาออก" htmlFor="trade-exit-price" error={errors.exitPrice?.message}>
          <input
            id="trade-exit-price"
            type="number"
            step="any"
        ...
        <FormField label="วันที่ออก" htmlFor="trade-exit-date" error={errors.exitDate?.message}>
          <input
            id="trade-exit-date"
            type="date"
            {...register("exitDate")}
      ...
      <FormField label="เวลาออก" htmlFor="trade-exit-time">
        <input
          id="trade-exit-time"
          type="time"
          {...register("exitTime")}
