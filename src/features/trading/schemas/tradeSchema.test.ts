...
describe("tradeSchema", () => {
  it("accepts a valid open trade", () => {
    expect(tradeSchema(t).safeParse(validOpenTrade).success).toBe(true);
  });

  it("rejects an empty symbol", () => {
    const result = tradeSchema(t).safeParse({ ...validOpenTrade, symbol: "" });
    expect(result.success).toBe(false);
  });

  it("rejects a non-positive entry price", () => {
    const result = tradeSchema(t).safeParse({ ...validOpenTrade, entryPrice: 0 });
    expect(result.success).toBe(false);
  });

  it("rejects a non-positive quantity", () => {
    const result = tradeSchema(t).safeParse({ ...validOpenTrade, quantity: -1 });
    expect(result.success).toBe(false);
  });

  it("rejects an unknown market", () => {
    const result = tradeSchema(t).safeParse({ ...validOpenTrade, market: "beanie-babies" });
    expect(result.success).toBe(false);
  });
  ...
  describe("closed trade requirements", () => {
    it("accepts a closed trade with exit price and date", () => {
      const result = tradeSchema(t).safeParse({
        ...validOpenTrade,
        status: "closed",
        exitPrice: 110,
        exitDate: "2026-07-22",
