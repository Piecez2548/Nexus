  ...
  return (
    <Drawer open={open} onClose={onClose}>
      <div className="space-y-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6">
        ...
        <div className="space-y-2 rounded-xl border border-brand-200 bg-brand-50/60 p-4 dark:border-brand-900/60 dark:bg-brand-950/20">
          <button
            type="button"
            onClick={() => onConfirm([])}
            className="w-full rounded-xl py-3 font-semibold transition active:scale-[0.99] nexus-primary-action"
          >
            {t("slipScanner.bankSelect.scanAll")}
          </button>
          <p className="text-center text-xs text-zinc-600 dark:text-zinc-400">
            {t("slipScanner.bankSelect.scanAllHint")}
          </p>
        </div>
        ...
        <button
          ...
          aria-controls="slip-scan-filters"
          className="flex w-full items-center justify-between rounded-xl border border-zinc-300 dark:border-zinc-700 px-4 py-3 text-left text-sm font-medium transition hover:bg-zinc-100 dark:hover:bg-zinc-800"
        >
          <span>{showFilters ? t("slipScanner.bankSelect.hideFilters") : t("slipScanner.bankSelect.advancedOptions")}</span>
          {showFilters ? <ChevronUp size={18} aria-hidden="true" /> : <ChevronDown size={18} aria-hidden="true" />}
        </button>

        {showFilters && <div id="slip-scan-filters" className="space-y-4">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{t("slipScanner.bankSelect.advancedOptionsHint")}</p>
          ...
          <button
            ...
            className="w-full rounded-xl py-3 font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 nexus-secondary-action"
          >
