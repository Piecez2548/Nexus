...
export function detectWeekendHabit(context: BehaviorEngineContext): DetectedHabit | null {
  const flag = context.behaviorAnalysis.flags.find((f) => f.key === "weekendSpending");
  if (!flag) return null;

  return detectFlagHabit(flag, recentWindowExpense(context.cashFlowAnalysis), "weekend", {
