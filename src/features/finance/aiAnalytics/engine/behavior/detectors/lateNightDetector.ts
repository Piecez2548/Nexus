...
export function detectLateNightHabit(context: BehaviorEngineContext): DetectedHabit | null {
  const flag = context.behaviorAnalysis.flags.find((f) => f.key === "nightSpending");
  if (!flag) return null;

  // detectFlagHabit already excludes dataQuality "unavailable" — most
  // transactions don't carry the optional `time` field, so this is the
  // common case, not an edge case.
  return detectFlagHabit(flag, recentWindowExpense(context.cashFlowAnalysis), "lateNight", {
